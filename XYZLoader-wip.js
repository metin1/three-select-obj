...
//TODO! use a factory and get the correct type!
	...
	createMesh() {
		...
		let partAttribute = new THREE.InterleavedBuffer(this.vertexBuffer, 6);
		let colorAttribute = new THREE.BufferAttribute(new Uint8Array(this.colorBuffer.buffer), 4, true);
    // console.log(`LL: XYZLoader -> createMesh -> colorAttribute`, colorAttribute)
    // console.log(`LL: XYZLoader -> createMesh -> this.colorBuffer.buffer`, this.colorBuffer.buffer)
		...
		geometry.setAttribute( 'part', new THREE.InterleavedBufferAttribute( partAttribute, 3, 3, true ));
...
//to be added back with colors
	...
	buildBuffers() {
		...
		this.partBuffer = new Uint32Array(indexSize);
		...
		for(let block of Object.values(this.blocks)) {
			...
			this.partBuffer[indexCount] = geometryId;
			geometryId++;
		}
	}
};
